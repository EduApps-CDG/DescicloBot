	...
	execute: async function(msg) {
		...
		switch (m[0]) {
			...
			case p + 'diga-no-canal':
				if (!m[2] || m[1].includes("474975924803338254")) {
					msg.channel.send("Por favor, faz direito!");
					msg.channel.send("Aliás, eu não vou mandar mensagens no <#474975924803338254>");
				} else {
					var txt = "";
			...
			//função prefix
			case p + 'prefix':
				if (!m[1] || !msg.member.hasPermission("ADMINISTRATOR")) {
			...
			case p + 'regras':
				if (msg.member.hasPermission("ADMINISTRATOR") || msg.author.id === "389292303933767690") {
					Regras.start(msg,m);
				} else {
			...
			case p + 'castigar':
				...
				if (pode || msg.member.hasPermission("ADMINISTRATOR") || msg.author.id === "389292303933767690") {
					// 775537028179296318
					...
					if (m[2]) {
						msg.channel.send("Castigado " + m[1] + " por " + m[2] + " horas, deixa comigo!");
						...
						msg.channel.send("Castigado " + m[1] + " por tempo indeterminado...\n**NÃO VÁ ESQUECER!!**");
					}
			...
			case p + 'descastigar':
				...
				if (pode || msg.member.hasPermission("ADMINISTRATOR") || msg.author.id === "389292303933767690") {
					...
					account.roles.add("311359153807294465").catch(console.error);
					if (m[2]) {
						msg.channel.send("Eu descastiguei " + m[1] + " por " + m[2] + " horas, quando o tempo for excedido, ele vai voltar pro cilindro!");
						setTimeout(async function() {
							account.roles.add("543499663806824497","porque eu posso!");
							account.roles.remove("311359153807294465").catch(console.error);
						}, parseFloat(m[2]) * 3600000);
					} else {
						msg.channel.send(m[1] + " foi descastigado");
					}
	...
	if (true) {
		...
		setTimeout(function() {
			...
			var r = request.get(link, function(err, res, body) {
				...
				msg.guild.channels.cache.get("775064669009215488").send(res.request.uri.href);
			});
