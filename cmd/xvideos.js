	...
	start: async function(msg,m) {
		if (msg.channel.nsfw) {
			...
			if(m[1] == "q" || m[1] == "query") {
				...
				msg.channel.send(carregador).then(function(msgbot) {
					...
					getVideosList2(pesquisa.slice(0,-1)).then((videos) => {
						...
						getDetails(videos[random].link).then((details) => {
							...
							msgbot.edit(modelo(details)).catch(console.error);
						});
					});
				});
			} else {
				msg.channel.send(carregador).then(async function(msgbot) {
